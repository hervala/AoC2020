...
{
    [Command(nameof(Day14))]
    public class Day14 : DayCommand, ICommand
    {
            ...
            foreach (var inputLine in day14Input.Split(Environment.NewLine).Where(s => !string.IsNullOrWhiteSpace(s)))
            {
                ...
                if (memMatch.Success)
                {
                    ...
                    for (int i = 0; i < mask.Length; i++)
                    {
                        if (reversedMask[i] != 'X')
                        {
                            reversedMask[i] = reversedMask[i] switch
                            {
                                '0' => (memIndex & (1UL << i)) != 0 ? '1' : '0',
                                '1' => '1',
                                _ => reversedMask[i],
                            };
                        } 
                    ...
                    var floatingCnt = reversedMask.Count(c => c == 'X');
                    var nthBit = -1;
                    var newMemIndex = 0UL;
                    foreach (var item in Enumerable.Range(0, (int)Pow(2,floatingCnt)))
                    {
                        newMemIndex = 0UL;
                        nthBit = -1;
                        for (int i = 0; i < mask.Length; i++)
                        {
                            switch (reversedMask[i])
                            {
                                case '0':
                                    newMemIndex &= ~(1UL << i);
                                    break;
                        ...
                        if (!mem.TryAdd(newMemIndex, value))
                        {
                            mem[newMemIndex] = value;
                        }
                    }
                }
            }

            console.Output.WriteLine($"{mem.Aggregate(0UL, (accu, next) => accu + next.Value)}");
            return default;
        }
        ...
        private string day14Input = @"mask = X101011X011X10101011000001X00XX0X000
mem[60126] = 9674686
mem[39254] = 523988
mem[54849] = 40771927
mem[29690] = 10110
