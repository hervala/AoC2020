...
{
    [DebuggerDisplay("Name = {Name}, Amount = {Amount}")]
    public record Bag 
    { 
        public string Name { get; init; }
        public List<Bag> CanBeIn { get; init; }
        public List<Bag> CanHave { get; init; }
    ...
    public class Day07 : DayCommand, ICommand
    {
        ...
        {
            var bagBuckets = ParseInput(day7Input);

            var found = new List<string>();
            if (bagBuckets.TryGetValue("shiny gold", out var TheGoldenBag))
            {
                foreach (var parent in TheGoldenBag.CanBeIn)
                {
                    TraverseParents(parent, found);
                }
            }

            console.Output.WriteLine($"{found.Distinct().Count()}");
            return default;
        }

        public override ValueTask Part02(IConsole console)
        {
            var bagBuckets = ParseInputV2(day7Input);
            var amount = 0;

            if (bagBuckets.TryGetValue("shiny gold", out var TheGoldenBag))
            {
                foreach (var child in TheGoldenBag.CanHave)
                {
                    var aa = CountAmounts(child, bagBuckets) * child.Amount;
                    amount += aa;
                }
            }

            console.Output.WriteLine($"{amount}");
            return default;
        }

        private void TraverseParents(Bag bag, List<string> found)
        {
            foreach (var parent in bag.CanBeIn)
            {
                TraverseParents(parent, found);
            }
            found.Add(bag.Name);
        ...
        private int CountAmounts(Bag bag, Dictionary<string, Bag> bags)
        {
            var amount = 0;
            if (bags.TryGetValue(bag.Name, out var bagFound))
            {
                if (bagFound.CanHave.Count() > 0)
                {
                    foreach (var item in bagFound.CanHave)
                    {
                        var aa = CountAmounts(item, bags) * item.Amount;
                        amount += aa;
                    }
                }
                else
        ...
        {
            var matcher = new Regex(@"(\d+)\s(\w+\s\w+\s\w+)|(no\sother\sbags)");
            Dictionary<string, Bag> bags = new();

            foreach (var item in input.Split(Environment.NewLine).Where(s => !string.IsNullOrWhiteSpace(s)))
            {
                var containsCut = item.Split("contain");
                var bagName = containsCut[0].Replace("bags", string.Empty).Replace("bag", string.Empty).Trim();

                if (!bags.TryGetValue(bagName, out var parentBag))
                {
                    parentBag = new Bag
                    {
                        Name = bagName,
                        CanBeIn = new List<Bag>(),
                        CanHave = new List<Bag>(),
                    };
                    bags.Add(bagName, parentBag);
                }

                var contents = containsCut[1].Split(",");
                foreach (var content in contents)
                {
                    ...
                    if (match.Success)
                    {
                        ...
                        {
                            bagName = match.Groups[2].Value.Replace("bags", string.Empty).Replace("bag", string.Empty).Trim().Trim('.');
                            if (!bags.TryGetValue(bagName, out var childBag))
                            {
                                childBag = new Bag
                                {
                                    Name = bagName,
                                ...
                                bags.Add(bagName, childBag);
                            }

                            parentBag.CanHave.Add(childBag);
                            childBag.CanBeIn.Add(parentBag);
                        }                        
                    }
                }
            }
        ...
        private Dictionary<string, Bag> ParseInputV2(string input)
        {
            var matcher = new Regex(@"(\d+)\s(\w+\s\w+\s\w+)|(no\sother\sbags)");
            Dictionary<string, Bag> bags = new();
        ...
        private string day7Input = @"dark olive bags contain 2 muted brown bags, 1 mirrored tomato bag, 4 bright black bags.
