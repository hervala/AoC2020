...
namespace AdventOfCode2020
{
    ...
    {
        private Regex parser = new Regex(@"^(\d+)\-(\d+)\s(\w)\:\s(.*)$"); 

        public override ValueTask Part01(IConsole console)
        {
            var validPwdCnt = 0;

            foreach (var (min, max, chr, pwd) in ParseInput(day2Input))
            {
                var cnt = pwd.Count(c => c == chr);
                if (cnt >= min && cnt <= max)
                {
                    validPwdCnt++;
                }
            ...
            console.Output.WriteLine($"{validPwdCnt}");
            return default;
        }
        ...
        public override ValueTask Part02(IConsole console)
        {
            ...
            foreach (var (pos1, pos2, chr, pwd) in ParseInput(day2Input))
            {
                if (pwd[pos1-1] == chr ^ pwd[pos2-1] == chr)
                {
                    validPwdCnt++;
                }
            }
        ...
        private IEnumerable<(int, int, char, string)> ParseInput(string input) => input
            .Split(Environment.NewLine)
            .Where(s => !string.IsNullOrWhiteSpace(s))
            .Select(row =>
            {
                var parsed = parser.Match(row);
                if (parsed.Success)
                {
                    return (int.Parse(parsed.Groups[1].Value), int.Parse(parsed.Groups[2].Value), parsed.Groups[3].Value[0], parsed.Groups[4].Value);
                }
                else
                {
                    throw new InvalidOperationException("Cannot parse " + row);
                }
            });

        private string day2Input = @"1-4 n: nnnnn
5-7 z: qhcgzzz
7-11 m: mmmmmmsmmmmm
